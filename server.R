...
server <- function(input, output, session) {
  ...
  #OUTCOME TYPE LOLLIPOP
  
  output$outcomeLollipop <- renderPlotly({
    plot_data <- filtered_data() %>%
      mutate(`Outcome Type` = case_when(
        `Outcome Type` %in% c("Transfer", "Transport") ~ "Transfer",
        `Outcome Type` %in% c("Disposal", "Died") ~ "Died (Not Euthanized)",
        `Outcome Type` == "Duplicate" ~ NA_character_,
        TRUE ~ `Outcome Type`
      )) %>%
    ...
    p <- ggplot(plot_data, aes(
      x = n,
      y = reorder(`Outcome Type`, n),
      text = paste("Outcome:", `Outcome Type`, "<br>Count:", n)
    )) +
      geom_segment(aes(x = 0, xend = n, y = `Outcome Type`, yend = `Outcome Type`),
                   color = "#999999", linewidth = 1) +
      geom_point(size = 8, color = "#4F9484") +
      labs(
  ...
  output$intakeReasons <- renderPlotly({
    reason_counts <- filtered_data() %>%
      ...
      mutate(`Reason for Intake` = as.character(`Reason for Intake`))  
    
    p <- condition_counts %>%
      mutate(grouped_condition = str_to_title(grouped_condition)) %>%
      ggplot(aes(
        x = n,
        y = grouped_condition,
        fill = grouped_condition
      )) +
      geom_col(aes(text = paste0("Condition: ", grouped_condition, "<br>Count: ", n))) +
      theme_minimal() +
  ...
  output$intakeConditionsOverview <- renderPlotly({
    condition_counts <- df_clean %>%
      ...
      mutate(grouped_condition = as.character(grouped_condition))
  ...
  outcome_colors <- c(
    "Adoption"                = "#3CB371",   # ✅ stay: MediumSeaGreen
    "Rescue Organization"     = "#0077B6",   # deep ocean blue
    "Rescue"                  = "#0077B6",   # same as above
    "Transferred"             = "#E76F51",   # bold orange (vs yellow)
    "Return To Owner"         = "#F9C74F",   # cleaner, lighter yellow
    "Return To Wild Habitat"  = "#43AA8B",   # minty forest green
    "Euthanasia"              = "#B22222",   # ✅ stay: Firebrick red
    "Died (Not Euthanized)"   = "#8B0000",   # ✅ stay: Deep red
    "Community Cat"           = "#9D4EDD",   # bold violet
    "Foster"                  = "#FF6F61",   # coral pink – nurturing
    "Other"                   = "#A0AEC0"    # soft slate gray
  )
  ...
  output$monthlyOutcomePlot <- renderPlotly({
    ...
    plot_data <- filtered_data() %>%
      ...
      mutate(
        Outcome = factor(Outcome, levels = names(outcome_colors)),  # ⬅️ this line is key
        text = paste0(
  ...
  #TOP CONDITIONS OF ANIMALS ENTERING SHELTER
  #HAD TO GROUP FOR CLEANLINESS
  output$groupedConditionsPlot <- renderPlotly({
    plot_data <- filtered_data() %>%
      filter(!is.na(grouped_condition)) %>%
      count(grouped_condition) %>%
      mutate(grouped_condition = forcats::fct_reorder(grouped_condition, n))
    
    color_palette <- c(
      "Underage/Underweight"    = "#E69F00",
      "Normal"                  = "#56B4E9",
      "Illness (Mild/Moderate)" = "#009E73",
      "Injured (Mild/Moderate)" = "#F0E442",
      "Injured (Severe)"        = "#D55E00",
      "Illness (Severe)"        = "#CC79A7",
      "Other"                   = "#999999",
      "Behavioral Issues"       = "#0072B2"
    )
    
    p <- ggplot(plot_data, aes(
      x = grouped_condition,  # now correctly a factor
      y = n,
      fill = grouped_condition
    )) +
      geom_col(aes(text = paste0("Condition: ", grouped_condition, "<br>Count: ", n))) +
  ...
  output$intakeTypePlot <- renderPlotly({
    plot_data <- filtered_data() %>%
      filter(!is.na(`Intake Type`)) %>%
      count(`Intake Type`) %>%
      mutate(`Intake Type` = forcats::fct_reorder(`Intake Type`, n))
    
    intake_type_colors <- c(
      "STRAY"            = "#0072B2",
      "WILDLIFE"         = "#D55E00",
      "OWNER SURRENDER"  = "#009E73",
      "WELFARE SEIZED"   = "#CC79A7",
      "RETURN"           = "#E69F00",
      "QUARANTINE"       = "#F0E442",
      "OTHER"            = "#999999"
    )
    ...
    p <- ggplot(plot_data, aes(
      ...
      geom_col(aes(text = paste0("Intake Type: ", `Intake Type`, "<br>Count: ", n)), show.legend = FALSE) +
      scale_fill_manual(values = intake_type_colors) +
      labs(
